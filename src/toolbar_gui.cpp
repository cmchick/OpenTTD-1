...
public:
	...
	void Draw(int left, int right, int top, int bottom, bool sel, int bg_colour) const
	{
		...
		DrawString(left + WD_FRAMERECT_LEFT + (rtl ? 0 : 8 + this->icon_size.width), right - WD_FRAMERECT_RIGHT - (rtl ? 3 + this->icon_size.width : 0), top + text_offset, STR_COMPANY_NAME_COMPANY_NUM, col);
	}
};
...
struct MainToolbarWindow : Window {
	...
	virtual void OnPaint()
	{
		...
		this->SetWidgetsDisabledState(Company::GetNumItems() == 0, WID_TN_STATIONS, WID_TN_FINANCES, WID_TN_CARGOS, WID_TN_TRAINS, WID_TN_ROADVEHS, WID_TN_SHIPS, WID_TN_AIRCRAFTS, WIDGET_LIST_END);
